...
DegorasASI installs a single prefix-relative layout (via `GNUInstallDirs`) that serves **both** a standard
...
cmake --install "$DEVSYSTEM_BUILDTREES/DegorasASI/mingw-dynamic-rel"
```

The preset already sets the install prefix to `$DEVSYSTEM_DEPLOYS/DegorasASI/<preset>/`, so no `--prefix` is
...
| Path | Contents |
|------|----------|
| `bin/` | `DegorasASI.dll` and (opt-out) the vendored ZWO `ASICamera2` runtime |
| `lib/` | import/static library |
| `include/DegorasASI/` | public headers, including the extensionless `Modules/` aggregators |
...
|--------|---------|--------|
| `DEGORASASI_INSTALL` | ON if top-level | Master switch for all install/export rules |
| `DEGORASASI_BUILD_SHARED` | follows `BUILD_SHARED_LIBS`, else ON | Shared vs static |
| `DEGORASASI_INSTALL_ASI_RUNTIME` | ON | Install the vendored ZWO runtime library into `bin/` |
| `DEGORASASI_BUILD_DOCS` | OFF | Install `README.md` into the doc dir |
| `DEGORASASI_INSTALL_EXAMPLES` | OFF | Install the example sources |
| `DEGORASASI_BUILD_TESTING` | ON | Build tests and register them with CTest |
| `DEGORASASI_BUILD_EXAMPLES` | ON | Build the example executables |
...
- `ports/degoras-asi/{vcpkg.json,portfile.cmake}` — the port.

### Manifest or classic mode

```sh
# classic mode
vcpkg install degoras-asi \
    --overlay-ports=DegorasASI/ports \
    --overlay-triplets=DegorasASI/cmake/triplets \
