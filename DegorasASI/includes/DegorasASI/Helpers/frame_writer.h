 ...
 */
DEGORASASI_EXPORT bool writeBmp(const types::Frame& frame, const std::string& path);
 ...
 */
DEGORASASI_EXPORT bool writePgm(const types::Frame& frame, const std::string& path);
...
struct DEGORASASI_EXPORT FitsCard
{
 ...
 */
DEGORASASI_EXPORT FitsCard fitsBayerPattern(types::BayerPattern pattern);
 ...
 */
DEGORASASI_EXPORT bool writeFrame(const types::Frame& frame, const std::string& path);

/// @brief The conventional file extension for the format, without a dot: "bmp" for RGB24, "pgm" otherwise.
DEGORASASI_EXPORT std::string extensionFor(types::ImageFormat format);
