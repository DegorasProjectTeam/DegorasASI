 ...
 *       information.
 */
DEGORASASI_EXPORT types::OperationResult categoryFromAsi(int asi_code);

/// @brief Human-readable name for a raw ZWO/ASI return code, or "ASI_UNKNOWN_ERROR" if unrecognised.
DEGORASASI_EXPORT std::string asiErrorToString(int asi_code);
 ...
 */
DEGORASASI_EXPORT types::DeviceError errorFromAsi(int asi_code, const std::string& context);
 ...
 */
DEGORASASI_EXPORT types::DeviceError errorFromLibrary(types::OperationResult category, const std::string& context);

/// @brief A success DeviceError (category OPERATION_OK, no raw code, empty context).
DEGORASASI_EXPORT types::DeviceError successError();
